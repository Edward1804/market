...
class CreatesitemapController extends BaseAdmin
{
    ...
    protected function parsing($urls){
        ...
        foreach ($urls as $i => $url){
            ...
            if(!preg_match('/Content-Type:\s+text\/html/i', $result[$i])){
                ...
                $this->cancel(0, 'Incorrect content ' . $url);
    ...
    protected function createLinks($content){
        ...
        if($content){
            ...
            preg_match_all('/<a\s*?[^>]*?href\s*?=\s*?(["\'])(.+?)\1[^>]*?>/i', $content, $links);
    ...
    protected function checkParsingTables(){
